 ...
 */
public class RHVTFragment extends Fragment {
...
*/
    ...
    public View onCreateView(
        ...
        final List <Kita> rhvtSchoolList= new ArrayList<>();


        schoolList = myAppDatabase.mMyDao().getRHVTSchools();

        String rhvtSchool = "";
        // יוצר רשימת בתי ספר בעיר חולון מהסטיודנטרום ומכניס לתוך רשימה מסוג קיטה
        for (StudentRoom sr : schoolList) {
            while (!sr.getSchool().equals(rhvtSchool) && !sr.getSchool().equals("-")) {
                rhvtSchool = sr.getSchool();
                rhvtSchoolList.add(new Kita("RHVT", rhvtSchool));
        ...
        mSchoolListAdapter = new SchoolListAdapter(rhvtSchoolList);
